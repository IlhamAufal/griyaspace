            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-brand-500 text-white">
                    <tr>
                        ...
                        <th class="px-6 py-3.5 text-left text-xs font-bold text-white uppercase tracking-wider">Waktu</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 text-sm text-gray-500 max-w-[200px] truncate" title="{{ $booking->activity_name }}">{{ $booking->activity_name }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $booking->booking_date->format('d/m/Y') }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $booking->start_time }} - {{ $booking->end_time }}</td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <div class="flex items-center gap-1">
                                <a href="{{ route('bookings.show', $booking) }}" class="inline-flex items-center justify-center w-8 h-8 text-blue-600 hover:text-blue-900 hover:bg-blue-50 rounded-lg transition-colors" title="Lihat Detail">
                                    <i class="fa-solid fa-eye text-sm"></i>
                                </a>
                                <button type="button" onclick="openDecisionModal('{{ $booking->id }}', '{{ $booking->booking_number }}', '{{ $booking->activity_name }}')" class="inline-flex items-center justify-center w-8 h-8 text-green-600 hover:text-green-900 hover:bg-green-50 rounded-lg transition-colors" title="Keputusan">
                    ...
                    @empty
                    <tr>
                        <td colspan="9" class="px-6 py-4 text-center text-sm text-gray-500">Tidak ada pengajuan yang perlu dikonfirmasi</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
...
function openDecisionModal(bookingId, bookingNumber, activityName) {
    ...
    document.getElementById('decisionSubmitText').textContent = 'Pilih Keputusan';
...
function setDecision(action) {
    ...
    if (action === 'approve') {
        ...
        btn.className = 'inline-flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-medium shadow-theme-xs transition-colors text-white bg-orange-600 hover:bg-orange-700';
