{{-- Step 2: Kalender & Pemilihan Tanggal --}}
<div x-show="step === 2" x-transition:enter="transition ease-out duration-300"
    ...
    {{-- Selected Dates & Time Summary Card --}}
    <div class="mt-5 p-5 bg-brand-50/40 dark:bg-brand-500/5 rounded-xl border border-brand-200/80 dark:border-brand-500/20">
        ...
        <div x-show="startDate" class="space-y-3">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                <div class="p-3 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-2xs">
                    <span class="text-gray-400 dark:text-gray-500 block mb-0.5 font-medium text-[11px]">Tanggal Mulai</span>
                    <p class="font-bold text-gray-800 dark:text-white" x-text="formatDateShort(startDate)"></p>
                </div>
                <div class="p-3 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-2xs">
                    <span class="text-gray-400 dark:text-gray-500 block mb-0.5 font-medium text-[11px]">Tanggal Akhir</span>
                    <p class="font-bold text-gray-800 dark:text-white" x-text="formatDateShort(endDate || startDate)"></p>
                </div>
                <div class="p-3 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-2xs">
                    <span class="text-gray-400 dark:text-gray-500 block mb-0.5 font-medium text-[11px]">Waktu Pelaksanaan</span>
                    <p class="font-bold text-brand-600 dark:text-brand-400"
                        x-text="startTime && endTime ? (startTime + ' - ' + endTime + ' WIB') : 'Belum ditentukan'"></p>
                </div>
                <div class="p-3 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-2xs">
                    <span class="text-gray-400 dark:text-gray-500 block mb-0.5 font-medium text-[11px]">Durasi / Hari</span>
            ...
            <template x-if="startDate && startTime && endTime">
                <div class="pt-2 border-t border-brand-200/60 dark:border-brand-500/20 flex flex-wrap items-center justify-between gap-2 text-xs">
                    <div class="flex items-center gap-2 text-brand-900 dark:text-brand-200 font-medium">
                        ...
                        <span>
                            ...
                            <b x-text="formattedDateRange"></b>, pukul <b x-text="startTime"></b> - <b x-text="endTime"></b> WIB
                        </span>
                    </div>
                </div>
            </template>
    ...
    <div x-show="showScheduleModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
        ...
        <div class="flex min-h-screen items-center justify-center p-4">
            <div class="relative w-full max-w-lg rounded-2xl bg-white dark:bg-gray-800 shadow-2xl border border-gray-100 dark:border-gray-700/60 overflow-hidden transform transition-all"
                ...
                {{-- Modal Header --}}
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700/60 bg-brand-50/50 dark:bg-brand-900/10">
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 rounded-xl bg-brand-500 text-white flex items-center justify-center shadow-xs">
                            <i class="fa-solid fa-calendar-days text-sm"></i>
                        </div>
                        <div>
                            <h3 class="text-base font-bold text-gray-900 dark:text-white">Tanggal & Waktu Peminjaman</h3>
                            <p class="text-[11px] text-gray-500 dark:text-gray-400">Atur rentang tanggal pelaksanaan dan jam peminjaman</p>
                        </div>
                    </div>
                ...
                <div class="p-6 space-y-4">
                    {{-- 2 Tanggal Inputs --}}
                    <div>
                        <label class="block text-xs font-semibold text-gray-800 dark:text-gray-200 mb-2">Tanggal Peminjaman</label>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                                ...
                                <p x-show="stepErrors.start_date" x-text="stepErrors.start_date" class="text-red-500 text-[11px] mt-1" style="display:none;"></p>
                            </div>
                            <div>
                                <label class="block text-[11px] font-medium text-gray-600 dark:text-gray-400 mb-1">
                                ...
                                <input type="date" x-model="endDate" :min="startDate || '{{ date('Y-m-d') }}'"
                                    class="h-10 w-full rounded-lg border border-gray-300 bg-white dark:bg-gray-900 px-3 py-2 text-sm text-gray-800 dark:text-white focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 shadow-xs">
                                <p x-show="stepErrors.end_date" x-text="stepErrors.end_date" class="text-red-500 text-[11px] mt-1" style="display:none;"></p>
                            </div>
                        </div>
                        ...
                        <div class="mt-2 text-xs flex items-center justify-between text-gray-500 dark:text-gray-400">
                            <span>
                                <span x-show="startDate && endDate">
                                    Total: <b class="text-brand-600 dark:text-brand-400" x-text="totalDays + ' hari'"></b>
                    ...
                    {{-- 2 Jam Inputs --}}
                    <div>
                        <label class="block text-xs font-semibold text-gray-800 dark:text-gray-200 mb-2">Waktu / Jam (Berlaku setiap hari peminjaman)</label>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                                <label class="block text-[11px] font-medium text-gray-600 dark:text-gray-400 mb-1">
                                ...
                                @enderror
                                <p x-show="stepErrors.start_time" x-text="stepErrors.start_time" class="text-red-500 text-[11px] mt-1" style="display:none;"></p>
                            </div>
                            <div>
                                <label class="block text-[11px] font-medium text-gray-600 dark:text-gray-400 mb-1">
                                ...
                                @error('end_time')
                                    <p class="text-red-500 text-[11px] mt-1">{{ $message }}</p>
                                @enderror
                                <p x-show="stepErrors.end_time" x-text="stepErrors.end_time" class="text-red-500 text-[11px] mt-1" style="display:none;"></p>
                            </div>
                        </div>
                        ...
                        <div class="mt-2 flex flex-wrap items-center justify-between gap-2 text-xs text-gray-500 dark:text-gray-400">
                            <span class="text-[11px]">
                                Jam operasional: <span class="font-medium text-gray-700 dark:text-gray-300" x-text="roomOpenTime + ' - ' + roomCloseTime"></span>
                            </span>
                            <span x-show="startTime && endTime" class="text-[11px] font-semibold text-brand-600 dark:text-brand-400">
                ...
                {{-- Modal Footer --}}
                <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-100 dark:border-gray-700/60 bg-gray-50/50 dark:bg-gray-900/30">
                    <button type="button" @click="closeScheduleModal()"
